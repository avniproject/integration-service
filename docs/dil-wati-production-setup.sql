-- ============================================================
-- DIL Wati Integration — Production Setup
-- Target: Production Integration Service → Prod Avni (Prod org)
--
-- TODOs before running:
--   1. Replace <PROD_ORG_NAME> with actual DIL production org name
--   2. Replace <AVNI_USER> and <AVNI_PASSWORD> with DIL admin credentials on prod Avni
--   3. Replace <WATI_API_URL> and <WATI_API_KEY> with DIL Wati production credentials
--   4. Replace <WEEKLY_SURVEY_TEMPLATE_EN/TE/OR> with approved weekly_survey template names
--   5. Update cron times if needed for production schedule
--
-- Run Section A on: production avni_int DB
-- Run Section B on: prod Avni DB (Production org)
-- ============================================================


-- ============================================================
-- SECTION A: Run on production avni_int DB
-- ============================================================

-- Step 1: Create integration system entry
INSERT INTO public.integration_system (name, system_type, uuid, is_voided)
VALUES ('<PROD_ORG_NAME>', 'wati', uuid_generate_v4(), false);
...
FROM integration_system WHERE name = '<PROD_ORG_NAME>';
...
INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'int_env', 'prod', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';

-- 5:30am IST = 00:00 UTC (Spring 6-field cron: second minute hour day month dow)
INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'main.scheduled.job.cron', '0 0 0 * * ?', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';
...
INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.weekly_survey.template_name.te_IN', '<WEEKLY_SURVEY_TEMPLATE_TE>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';

INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.weekly_survey.template_name.od_IN', '<WEEKLY_SURVEY_TEMPLATE_OR>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';
...
INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.biweekly_payment.template_name.te_IN', '<BIWEEKLY_PAYMENT_TEMPLATE_TE>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';

INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.biweekly_payment.template_name.od_IN', '<BIWEEKLY_PAYMENT_TEMPLATE_OR>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';
...
INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.weekly_survey_rnd.template_name', '<WEEKLY_SURVEY_RND_TEMPLATE_EN>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';

INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.weekly_survey_rnd.template_name.te_IN', '<WEEKLY_SURVEY_RND_TEMPLATE_TE>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';

INSERT INTO integration_system_config (integration_system_id, key, value, uuid)
SELECT id, 'flow.weekly_survey_rnd.template_name.od_IN', '<WEEKLY_SURVEY_RND_TEMPLATE_OR>', uuid_generate_v4()
FROM integration_system WHERE name = '<PROD_ORG_NAME>';
...
-- ============================================================
-- SECTION B: Run on prod Avni DB (Production org)
-- ============================================================
...
VALUES (
 ...
 FROM encounter e
    ...
    (SELECT id FROM organisation WHERE name = '<PROD_ORG_NAME>'), false, 0, 1, 1, now(), now()
);
...
-- ============================================================
-- SECTION C: Verification (run on production avni_int DB after setup)
-- ============================================================

-- Confirm integration system and all config rows are present
SELECT key, value FROM integration_system_config
WHERE integration_system_id = (SELECT id FROM integration_system WHERE name = '<PROD_ORG_NAME>')
ORDER BY key;
